...
mixer.init() #инициализация звука
...
class Player(): #игрок
	def __init__(self, x, y, path, stop, coord, name, frame_count=1, scale=(100,80), flip=False, pathToMusic="None"): #координаты x y путь к картинке, остановлен, координаты по цифрам, имя
		self.x = x; self.y = y
		self.path = path
		self.stop = stop
		self.coord = coord
...
#создаем игроков
Gamers = [Player(1692, 900, "./images/horse/horse.png", \
			0, 0, "1", 13, (100, 80), True, \
			(["./music/horse" + str(i) + ".mp3" for i in range(1, 7)])),\
		Player(1718, 900, "./images/monkey/monkey.png", \
		0, 0, "2", 8, (200, 160), False,\
